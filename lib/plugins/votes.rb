  ...
  command(:votes) do |event|
    ...
    begin
      event.channel.send_embed do |embed|
        ...
        embed.colour = 0xd084
        embed.url = 'https://discordbots.org/bot/578544051901431821/vote'
        ...
        if event.bot.server(463_178_169_105_645_569).members.include? event.user
          ...
          embed.add_field(name: 'Your Current Vote Perks', value: 'Sorry, but you need to be on the [HQ Trivia Bot server](https://discord.gg/ugryabu) to get sweet perks.', inline: true)
        end
        unless CONFIG['dbotsorg'].nil?
          status = if DBL.stats.verifyvote(event.user.id)
                     'Nope! Thanks for voting :D'
                   else
                     'Yes! [Vote](https://discordbots.org/bot/578544051901431821/vote) for perks!'
                   end
