module AuthKey
  ...
  command(:authkey, min_args: 0) do |event, *args|
    ...
    memb = Bot.server(463178169105645569).member(event.user.id)
    if memb.nil?
      extra = "\n\nMake sure you join the official server for a role! https://discord.gg/ugryabu"
    else
      memb.add_role(473301704272773120)
    end

    Bot.channel(618115005346217986).send_embed do |embed|
